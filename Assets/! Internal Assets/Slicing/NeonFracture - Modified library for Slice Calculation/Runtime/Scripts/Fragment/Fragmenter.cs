...
public static class Fragmenter
{
                             ...
                             Transform parent)
    {
        ...
        int i = 0;
        var topFragments = CreateFragment(topSlice,
                       sourceObject,
                       fragmentTemplate,
                       parent,
                       false,
        ...
        var bottomFragments = CreateFragment(bottomSlice,
                       sourceObject,
                       fragmentTemplate,
                       parent,
                       false,
        ...
        // Создаем новый список для объединенных фрагментов
        List<GameObject> combinedFragments = new List<GameObject>();

        // Добавляем все элементы из topFragments в combinedFragments
        combinedFragments.AddRange(topFragments);

        // Добавляем все элементы из bottomFragments в combinedFragments
        combinedFragments.AddRange(bottomFragments);
                                       ...
                                       ref int i)
    {
        ...
        float parentMass;
        var rbComponent = sourceObject.GetComponent<Rigidbody>();
        if (rbComponent != null)
        {
            parentMass = rbComponent.mass;
        }
        else
        {
            parentMass = 1f;
