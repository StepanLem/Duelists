...
//Решения для стабилизации:
//1.При отпускании меча(но не ножен), у ножен скорость трэка поворота(и позиции тоже мб) уменьшить так, чтобы ножны выпрямились достаточно медленно, но не слишком.
//2.При отпускании ножен(но не меча), аналогичная ситуация, но уже меч возвращается в свою позицию мдленно.
//Оба прерываются, если ножны будут снова схвачены или если человек отпустит и второй предмет.
